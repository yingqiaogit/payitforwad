...
contract Wallet {
    ...
    event Redeem(address indexed _from, address indexed _to, uint256 amount);
    ...
    function removePromise(uint index) internal {
    ...
    function donate(address from, address payable to, uint256 amount) 
        public isOwner(from) sufficient(amount)  payable {
        to.transfer(amount);
        emit Donate(from, to, amount);        
    }

    function redeemPromise(address from, address payable to, uint index) 
            external isOwner(from) payable {
        Promise memory redeemed = promises[promiseNames[index]];
        
        to.transfer(redeemed.amount);
        require (address(this).balance >= redeemed.amount);
        emit Redeem(from, to, redeemed.amount);
